...
AI 对话式 5Why 分析工具：用户描述问题 → AI 依据 `promt.txt` 中的提示词逐层追问（一次一问、简洁、验证证据、拒绝个人归因）→ 直至找到可改善的根本原因。支持**双模式**：AI 追问模式（默认）与自答校验模式（点「＋ 解答」填上自己的解答，AI 先校验这一问一答是否合理、方向是否跑偏，再继续引导下一层）。支持多会话历史管理、导出 Markdown 报告。
...
- 自答校验：点输入框右侧「＋ 解答」填写自己的解答，问题 + 解答一并提交，AI 校验方向与合理性
